...
class EcoImpact:
    ...
    def __init__(self, page):
        ...
        self.counter_selector = "//div[contains(@class, \"desktop-impact-item\") and count(.//div[contains(@class, \"desktop-value\")]) = 1]"
    ...
    def check_text_eco_counter(self, counter_index: int, unit: str, label: str):
        ...
        try:
            ...
            measurement = self.page.locator("xpath=" + self.counter_selector + f"[{counter_index}]//div[contains(@class, \"desktop-unit\")]")
            expect(measurement).to_contain_text(unit)

            # Selector for description text
            description = self.page.locator("xpath=" + self.counter_selector + f"[{counter_index}]//div[contains(@class, \"desktop-label\")]")
    ...
    def check_eco_impact_not_auth(self, counter_index: int, opacity: str):
        ...
        3) Eco counters are disabled: their values equal zero and their texts are partially transparent.
        : param counter_index: number of an eco-counter card
        ...
        try:
            ...
            disability = self.page.locator("xpath=" + self.counter_selector + f"[{counter_index}]//div[contains(@class, \"desktop-disabled\")]")
            expect(disability).to_have_css("opacity", opacity)

            # Selector for value of a counter
            value = self.page.locator("xpath=" + self.counter_selector + f"[{counter_index}]//div[contains(@class, \"desktop-value\")]")
